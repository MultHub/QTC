...
local args = {...}
local nonce, blocks = 0, 0
local score = 400000000001
local target, last, sha256_hash
function recall()
	term.clear()
	term.setCursorPos(1,1)
	print("Hashes this run: "..tostring(nonce))
	print("Blocks this run: "..tostring(blocks))
	target = tonumber(http.get("http://coinserv.ceriat.net/c/qtc/index.php?getwork").readAll())
	last = http.get("http://coinserv.ceriat.net/c/qtc/index.php?lastblock").readAll()
...
while true do
	...
	status = http.get("http://coinserv.ceriat.net/c/qtc/index.php?submitblock&address="..args[1].."&nonce="..tostring(nonce)).readAll()
end
